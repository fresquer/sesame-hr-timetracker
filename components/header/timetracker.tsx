  ...
  useEffect(() => {
    if (trackingInfo) {
      ...
      if (trackingActive) {
        ...
        const time: string = calculateTime(
          trackingInfo.workEntryIn?.date,
          trackingInfo.workEntryOut?.date
        );
        setTrackingTime(time);
      }
    }
  ...
  return (
    <div className="timetracker_wrapper flex">
      ...
      <div className="buttons_wrapper flex">
        {!trackingActive ? (
          <button className="start" onClick={() => handleStartTracking()}>
            Entrar
          </button>
        ) : (
          <>
            <button className="pause">Pausar</button>
            <button className="end" onClick={() => handleEndTracking()}>
              Salir
            </button>
          </>
        )}
